...
float endFade(float t) { return smoothstep(0.0,0.018,t)*(1.0-smoothstep(0.974,1.0,t)); }
`;
...
void main() {
  ...
  vec3 rock=mix(vec3(0.13,0.18,0.16),vec3(0.32,0.32,0.25),grain);
...
void main() {
  ...
  vec3 water=mix(vec3(0.23,0.38,0.32),vec3(0.055,0.20,0.22),deep);
  ...
  float spec=pow(max(dot(N,H),0.0),exponent)*(1.0-rough)*0.4;
  float fresnel=0.025+0.38*pow(1.0-max(dot(N,V),0.0),5.0);
  water*=0.82+0.25*max(dot(N,L),0.0);
  water+=vec3(0.75,0.66,0.43)*spec+vec3(0.22,0.32,0.31)*fresnel;
  ...
  float foam=foamBand*smoothstep(0.28,0.7,breakup+pulse*0.18)*0.65;
  water=mix(water,vec3(0.75,0.79,0.68),foam);
  float alpha=max(depthFade*0.92,foam*0.85)*endFade(vUv.y);
