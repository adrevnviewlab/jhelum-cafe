...
export default function MenuCatalog({ category, setCategory, onAddItem, openCart }) {
    ...
    {visible.length ? visible.map(section => <section className="catalog-section" key={section.id} aria-label={section.title}>
      ...
      <div className={`catalog-items catalog-items--photographic ${['breakfast', 'bakery'].includes(section.id) ? '' : 'catalog-items--portrait'}`}>
        {section.items.map(item => {
          ...
          return <article key={item.id} className="catalog-item catalog-item--photographic">
            ...
            <div className="catalog-item-body">
              ...
              <div className="catalog-item-action">
                {item.cents == null
                  ? <a href={cafe.phoneHref}>Call for price <Icon /></a>
                  : <button type="button" onClick={() => onAddItem(cartItem, needsOptions)}>{needsOptions ? 'Choose options' : 'Add to order'} <Icon name="plus" /></button>}
              </div>
            </div>
          </article>;
        })}
